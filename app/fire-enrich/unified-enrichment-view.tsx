...
const PRESET_FIELDS: EnrichmentField[] = [
  ...
  // Decision Maker Fields - Our Key Differentiator
  { name: 'decisionMakers', displayName: 'Decision Makers', description: 'Key executives and decision makers at the company', type: 'array', required: false },
  { name: 'ceo', displayName: 'CEO', description: 'Chief Executive Officer information', type: 'string', required: false },
  { name: 'cto', displayName: 'CTO', description: 'Chief Technology Officer information', type: 'string', required: false },
  { name: 'cfo', displayName: 'CFO', description: 'Chief Financial Officer information', type: 'string', required: false },
  { name: 'headOfSales', displayName: 'Head of Sales', description: 'Head of Sales/Revenue information', type: 'string', required: false },
  { name: 'headOfMarketing', displayName: 'Head of Marketing', description: 'Head of Marketing information', type: 'string', required: false },
  { name: 'customDecisionMakers', displayName: 'Custom Decision Makers', description: 'Custom roles to search for (e.g., VP of Engineering, Director of Product)', type: 'string', required: false },
];
...
export function UnifiedEnrichmentView({ rows, columns, onStartEnrichment }: UnifiedEnrichmentViewProps) {
  ...
  const [selectedFields, setSelectedFields] = useState<EnrichmentField[]>([
    // Default selected fields - focused on decision makers
    PRESET_FIELDS.find(f => f.name === 'companyName')!,
    PRESET_FIELDS.find(f => f.name === 'decisionMakers')!,
    PRESET_FIELDS.find(f => f.name === 'ceo')!,
  ...
  const handleAddCustomRoles = () => {
    if (!customRoles.trim()) {
      toast.error("Please enter at least one role to search for");
      return;
    }

    // Add the custom roles field
    const customRolesField: EnrichmentField = {
      name: 'customDecisionMakers',
      displayName: 'Custom Decision Makers',
      description: `Search for these specific roles: ${customRoles}`,
      type: 'string',
      required: false
    };

    // Remove existing custom field if present
    const filteredFields = selectedFields.filter(f => f.name !== 'customDecisionMakers');
    handleAddField(customRolesField);

    setCustomRoles('');
    setShowCustomRoles(false);
    toast.success(`Will search for: ${customRoles}`);
  };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <Card className="p-4 border-orange-200 hover:border-orange-300 transition-all duration-300 dark:border-orange-900/30 dark:hover:border-orange-800/50">
                    ...
                    >
                      ...
                      {showCustomRoles ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                    </Button>

                    {showCustomRoles && (
                      <div className="mt-4 space-y-3">
                        <div className="text-sm text-muted-foreground mb-2">
                          Search for specific decision maker roles (e.g., "VP of Engineering", "Director of Product", "Head of Sales")
                        </div>
                        <Textarea
                          placeholder="Enter roles to search for (one per line or comma separated)"
                          value={customRoles}
                        ...
                        />
                        <Button
                          onClick={handleAddCustomRoles}
                          disabled={!customRoles.trim()}
                          variant="orange"
                          className="w-full"
                        >
